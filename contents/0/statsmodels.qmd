...
1. biased but consistent: example: sample variance with ddof=0 (It is also MLE of mean?)
2. unbiased but inconsistent: when the variance is not reduced when n grows: example: sample max is unbiased estimator of the mean in an uniform distribution U[0, t], but it does not converge to the upper bound t as n->infty.
...
The acronym BLUE stands for Best Linear Unbiased Estimator. It refers to an estimator that is:
...
Best: It has the minimum variance among all unbiased estimators. (efficient)
...
If these conditions hold, then the OLS estimator is the best linear unbiased estimator in terms of minimum variance.
